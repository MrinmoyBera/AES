...
void increment_ctr(unsigned char *ctr) {
    for (int i = 15; i >= 0; i--) {
        ctr[i]++;
        if (ctr[i] != 0) {
            break;
...
void ctr_encryption(const char *input_file_name, const char *ciphertext_file_name, unsigned char key[16]){
    ...
    fwrite(ctr, 1, 16, ciphertext_file);
    ...
    while ((bytesRead = fread(buffer, 1, 16, input_file)) == 16) {
       ...
       //Encrypted text
       unsigned char **encrypted_text = encryption(ctr, round_keys);
...
void ctr_decryption(const char *ciphertext_file_name, const char *plaintext_file_name_after_dec, unsigned char key[16]){
    ...
    fread(ctr,1,16,ciphertext_file);
...
int main() {
    ...
    ctr_encryption(input_file_name, ciphertext_file_name, key);	 
    ctr_decryption(ciphertext_file_name, plaintext_file_name_after_dec, key);
