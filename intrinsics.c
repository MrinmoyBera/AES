#include <stdio.h>
#include <wmmintrin.h> // Intel intrinsics for AES

// Helper function to print 128-bit state as hexadecimal
...
__m128i aes128_decrypt(__m128i ciphertext, const unsigned char *userkey) {
    ...
    for (int i = 9; i >= 1; i--) {
        ...
        state = _mm_xor_si128(state, ((__m128i*)key_schedule)[i]);   //InvMixColumns(InvSubBytes(InvShiftRows(state)))
